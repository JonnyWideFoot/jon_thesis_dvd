	...
	public sealed class RamachandranAnalysis : DSSPTaskDirecory_PhiPsiData
	{
		...
		private int DoRamachandranOrigin( int totalResiduesInAllDatabase, DSSPReportingOn doReportOn, DSSPIncludedRegions mode, StandardResidues resTypes )
		{
			...
			if( doReportOn == DSSPReportingOn.All )
			{
				...
				m_OutputFileStem = name + "_All";
				m_GraphTitle = "Residue type : \"" + name + "\" in all structures\r\n";
			}
			else if( doReportOn == DSSPReportingOn.LoopsOnly )
			{
				// make a filename based on the above params (**ignore the DSSPIncludedRegions for now )
				m_OutputFileStem = name + "_Loop";
				m_GraphTitle = "Residue type : \"" + name + "\" in loop regions only\r\n";
			}
			else
			{
			...
			PhiPsiUpdateOriginGraphs( useGraph, useMatrix, secondHighest * 2.5 );				
		...
		private void HTMLPhiPsiReportPictures( DSSPReportingOn DSSPReportingOn, string[] pageNames )
		{
			...
			for( int i = 0; i < pageNames.Length; i++ )
			{
				...
				string thumbName = m_OutputFileStem + "_T_" + pageNames[i] + ".jpg";
				if( "Scatter" == pageNames[i] )
				{
					InteractOrigin.SetDataPointPlotSize("Scatter","PhiPsi_b",3);
				}
				InteractOrigin.SavePicture( reportDirectory.FullName + thumbName, pageNames[i], 350, 350 );
				string imgName = m_OutputFileStem + "_B_" + pageNames[i] + ".jpg";
				if( "Scatter" == pageNames[i] )
				{
					InteractOrigin.SetDataPointPlotSize("Scatter","PhiPsi_b",1);
				}
				InteractOrigin.SavePicture( reportDirectory.FullName + imgName, pageNames[i], 1000, 1000 );
		...
		private void PhiPsiUpdateOriginGraphs( string contourName, int[,] matrix, double maxContourValue )
		{
			PhiPsiUpdateOriginm_GraphTitles();
			PhiPsiUpdateOriginGraphRanges( contourName, matrix, maxContourValue );
		}
