...
namespace UoB.Core.FileIO.DSSP
{
	...
	public class DSSPFile : BaseFileType_Reinitialisable
	{
        ...
        private const float TempFactorForCACutoff = 25.0f;
        // obtained from the "Dissalowed ramachandran regions in proteins" paper...
		...
		protected override void Initialise(string fileName)
		{
			...
			//{
				if( IsValidFile )
				{
					...
					if( line == null )
					{
                        throw new Exception("File doesnt contain the correct number of lines" + m_FileInfo.FullName);
					}
					if( line.Length < 40 )
					{
                        throw new Exception("Header line is not the required size" + m_FileInfo.FullName);
					}
					if( 0 != String.Compare( line, 0, "  #  RESIDUE AA STRUCTURE BP1 BP2  ACC  ", 0, 40, false ) )
					{
                        throw new Exception("The title line of the DSSP file did not contain the correct string" + m_FileInfo.FullName);
					}
		...
		{ 
            ...
            // Fecking hell DSSP is stupid
            if (Char.IsLower(def.AminoAcidID))
            {
                def.AminoAcidID = 'C'; // HACK!!
            }
		...
		private void CalculateSegments()
		{
            ...
            // Now look up the border types for each loop
            for (int i = 0; i < m_LoopDefs.Count; i++)
            {
                SegmentDef def = m_LoopDefs[i] as SegmentDef;
                if( def.Length > 0 )
                {
                    int index = def.FirstDSSPIndex;
                    if (index > 1)
                    {
                        def.NTerminalDSSPType = types[index - 2];
                    }
                    index = def.LastDSSPIndex;
                    if (index < types.Length)
                    {
                        def.CTerminalDSSPType = types[index];
                    }
                }
            }
		}
        ...
        private bool LoopPassesFilter(SegmentDef segment)
        {
            ...
            for (int i = 0; i < segment.Length; i++)
            {
                ...
                else if (segment[i].tempFactor > TempFactorForCACutoff)
                {
                    // the positions will be less certain
                    return false; // dont want it ...
                }
                else if (segment[i].occupancy != 1.0f)
                {
                    // dodgy files where the occupancy varies, ask sesh ...
                    return false;
                }
            }
		...
		public SegmentDef[] GetLoops( int length, bool includeTermini, bool validityFilter )
		{
            // Regarding including termini: 
            // Note that the N and C termini are always there at positions 0 and n-1 
            // even if the lengths are 0 ...
			...
			for( int i = startAt; i < endAt; i++ )
			{
                ...
                if (length == -1 || length == ld.Length)
				{
                    if (validityFilter && !LoopPassesFilter(ld))
                    {
                        continue;
                    }
					ar.Add( ld );
        ...
        public SegmentDef[] GetLoops(bool includeTermini, bool validityFilter)
		{
            return GetLoops(-1, includeTermini, validityFilter);
		}
