...
namespace UoB.Core.Structure
{
	...
	public class ParticleSystem : AtomList, ICloneable
	{
        ...
        public override void DeleteHydrogens()
        {
            BeginEditing();
            for (int i = 0; i < m_Members.Count; i++)
            {
                ...
                for (int j = 0; j < molContainer.Count; j++) // j increments the molecules in each container
                {
                    ...
                    while (k >= 0)
                    {
                        if (m[k].atomPrimitive.Element == 'H')
                        {
                            m.RemoveAtomAt(k); // RemoveAtomAt(k);
                        }
                        k--;
                    }
                }
        ...
        public void DetectDisulphides(int molIndex)
        {
            PSMolContainer mol = MemberAt(molIndex);
            if (mol == null) throw new ArgumentOutOfRangeException();

            for (int i = 0; i < mol.Count; i++)
            {
                if (0 == String.Compare(mol[i].Name_NoPrefix, "CYS", true))
                {
                    for (int j = i+1; j < mol.Count; j++)
                    {
                        if (0 == String.Compare(mol[j].Name_NoPrefix, "CYS", true))
                        {
                            Atom a = mol[i].AtomOfType(" SG ");
                            Atom b = mol[j].AtomOfType(" SG ");
                            if (a != null && b != null)
                            {
                                if (2.5 > a.distanceTo(b))
                                {
                                    // We have found ourselves a disulphide bond :-D
                                    mol[i].ResetName("CYX",true);
                                    mol[j].ResetName("CYX",true);
                                    a.bondTo(b);
                                    b.bondTo(a);
                                }
                            }
