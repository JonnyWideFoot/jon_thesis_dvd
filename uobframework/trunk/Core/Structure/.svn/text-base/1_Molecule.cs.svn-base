...
namespace UoB.Core.Structure
{
	...
	public class Molecule : AtomList, ICloneable
	{
        ...
        public void ResetName(string name, bool assignAtomPrimitives)
        {
            m_Name = name;
            setMolPrimitive(m_Prefix, assignAtomPrimitives);
        }
		...
		{
            if (m_MoleculePrimitive != null && (null != m_MoleculePrimitive as MoleculePrimitive) )
			{
        ...
        public override void RemoveAtomAt(int index)
        {
            this[index].parentMolecule = null;
            base.RemoveAtomAt(index);            
        }
		...
		public Atom AtomOfType( string PDBID )
		{
			for( int i = 0; i < m_Atoms.Count; i++ )
			{
				if( 0 == String.CompareOrdinal( ((Atom)m_Atoms[i]).PDBType, PDBID) )
				{
