...
namespace UoB.Core.MoveSets.AngleSets
{
	...
	public class AngleSet
	{
		...
		public AngleSet( SpecifiedAngleSet setName )
		{
			...
			switch( setName )
			{
				case SpecifiedAngleSet.Optimal6Loop:
                    fileName += "Optimal_6_Loop.angleset";
					break;
				case SpecifiedAngleSet.OriginalRAFT:
                    fileName += "Original_RAFT.angleset";
					break;
				default:
                    throw new NoImplementationException("Specified AngleSet found with no implemenation!");
			}
		...
		{
            Init(CoreIni.Instance.DefaultSharedPath + "Optimal_6_Loop.angleset");
		}
		...
		{
			double[] availPhis = GetPhis( molID, false );
			double[] availPsis = GetPsis( molID, false );
            bool[] isCis = GetIsCis(molID);
            return ClosestDistanceTo(Char.IsLower(molID), availPhis, availPsis, isCis, phi, psi);
		}

		public static double ClosestDistanceTo( bool cis, double[] availPhis, double[] availPsis, bool[] isCis, double phi, double psi )
		{
			double bestDistance = double.MaxValue;
			for( int i = 0; i < availPhis.Length; i++ )
			{
                if (cis != isCis[i]) continue;
		...
		{
            bool needCis = Char.IsLower(id);
            ResidueAngleSet rSet = this[Char.ToUpper(id)];
			if( rSet == null )
			{
				return -1;
			}
        ...
        {
            return GetPropensities(id, true);
        }

		public float[] GetPropensities( char id, bool segregateCisTrans )
		{
            bool needCis = Char.IsLower(id);
            if (!m_CaseSplitCis) id = Char.ToUpper(id);            
			...
			{
                List<float> propensity = new List<float>( rSet.AngleCount );
				for( int i = 0; i < rSet.AngleCount; i++ )
				{
                    if (needCis == rSet.getIsCis(i))
                    {
                        propensity.Add(rSet.getPropensity(i));
                    }
				}
				return propensity.ToArray();
			}
		}

        public double[] GetPhis(char id)
        {
            return GetPhis(id,true);
        }

		public double[] GetPhis( char id, bool segregateCisTrans )
		{
            bool needCis = Char.IsLower(id);
            if (!m_CaseSplitCis) id = Char.ToUpper(id);
			ResidueAngleSet rSet = this[id];
			...
			{                
                List<double> phis = new List<double>(rSet.AngleCount);
				for( int i = 0; i < rSet.AngleCount; i++ )
				{
                    if (!segregateCisTrans || needCis == rSet.getIsCis(i))
                    {
                        phis.Add(rSet.getPhi(i));
                    }
				}
				return phis.ToArray();
			}
        ...
        private bool m_CaseSplitCis = false;
        public bool CaseSplitCis
        {
            get
            {
                return m_CaseSplitCis;
            }
            set
            {
                m_CaseSplitCis = value;
            }
        }

        public bool[] GetIsCis(char id)
        {
            if( !m_CaseSplitCis ) id = Char.ToUpper(id);
            ResidueAngleSet rSet = this[id];
            if (rSet == null)
            ...
            {
                bool[] isCis = new bool[rSet.AngleCount];
                for (int i = 0; i < rSet.AngleCount; i++)
                {
                    isCis[i] = rSet.getIsCis(i);
                }
                return isCis;
            }
        }
        ...
        {
            return GetPsis(id, true);
        }

        public double[] GetPsis(char id, bool segregateCisTrans)
		{
            bool needCis = Char.IsLower(id);
            if (!m_CaseSplitCis) id = Char.ToUpper(id);
			ResidueAngleSet rSet = this[id];
			...
			{               
                List<double> psis = new List<double>(rSet.AngleCount);
				for( int i = 0; i < rSet.AngleCount; i++ )
				{
                    if (!segregateCisTrans || needCis == rSet.getIsCis(i))
                    {
                        psis.Add(rSet.getPsi(i));
                    }
				}
                return psis.ToArray();
			}
