...
//		}
        ...
        public void setToOptimalRotation(PSMolContainer mol1, PSMolContainer mol2)
        {
            if (mol1.Count != mol2.Count)
            {
                throw new Exception();
            }

            if (mol1.Atoms.Count != mol2.Atoms.Count)
            {
                throw new Exception();
            }

            // A bit of a hack - set direct equivelency between resudues i.e. 1:1 2:2 3:3 etc.
            int[] equiv = new int[mol1.Count];
            ...
            Position[] p1 = new Position[mol1.Atoms.Count];
            Position[] p2 = new Position[p1.Length];
            for( int i = 0; i < p1.Length; i++ )
            {
                p1[i] = new Position(mol1.Atoms[i]);
                p2[i] = new Position(mol2.Atoms[i]);
                if( 0 != String.Compare(mol1.Atoms[i].PDBType,mol2.Atoms[i].PDBType) )
                {
                    throw new Exception();
                }
            }           

            setToOptimalRotation( p1, p2, equiv );
        }
