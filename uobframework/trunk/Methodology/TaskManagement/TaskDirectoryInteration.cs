...
namespace UoB.Methodology.TaskManagement
{
	...
	{
		private StringBuilder m_FilenameMaker = new StringBuilder();
		...
		/// </summary>
		public TaskDirectoryInteration( DirectoryInfo diRoot, bool OriginIsRequired )
		{
			if( !diRoot.Exists ) throw new IOException("Directory root doesn't exist...");
            m_BaseDI = diRoot;
            m_ReportDir = new DirectoryInfo( m_BaseDI.FullName + "\\Report\\");
			if( !m_ReportDir.Exists ) m_ReportDir.Create();
			m_ResultDir = new DirectoryInfo( m_BaseDI.FullName + "\\Result\\");
			if( !m_ResultDir.Exists ) m_ResultDir.Create();     
			m_ScriptGenDir = new DirectoryInfo( m_BaseDI.FullName + "\\ScriptGen\\");
		...
		protected void HTMLReportingBegin( string reportTitle )
		{
            ...
            m_HTMLReporter.WriteLine("table { margin: auto; }");
            m_HTMLReporter.WriteLine("td { text-align: center; }\r\n</style>\r\n");
		...
		protected void HTMLStartReportBlock( string titleRow )
		{
			...
			m_HTMLReporter.WriteLine( "</h1></td></tr><tr><td>" );
		}

		protected void HTMLEndReportBlock()
		{
			AssertHTMLReportIsActive();
            m_HTMLReporter.WriteLine("</tr></td></table></center>");
            m_HTMLReporter.Flush();
		}
		...
		{			
			...
			{
                string replaceFilename = baseDirectory.FullName + "OriginOverride" + Path.DirectorySeparatorChar  + "_Template.opj";
                if (!File.Exists(replaceFilename))
                {
                    replaceFilename = String.Concat(UoB.Core.CoreIni.Instance.DefaultSharedDir.FullName, "OriginTemplate", Path.DirectorySeparatorChar, "histogram.opj");
                    if (!File.Exists(replaceFilename))
                    {
                        throw new Exception("Origins default template file was not found!");
                    }
                }
                try
                {
                    File.Copy(replaceFilename, templateFilename);
                }
                catch (Exception ex)
                {
                    throw new Exception("Couldnt copy origin template:\r\n" + ex.ToString());
                }                
			}
